...
public class BufferedReadChannel extends BufferedChannelBase implements Closeable {
    ...
    synchronized public int read(ByteBuf dest, long pos, int length) throws IOException {
        invocationCount++;
        ...
        while (length > 0) {
            ...
            if (readBufferStartPosition <= currentPosition && currentPosition < readBufferStartPosition + readBuffer.readableBytes()) {
