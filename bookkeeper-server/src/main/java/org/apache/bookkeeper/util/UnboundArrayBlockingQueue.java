 ...
 * <p/>
 * This queue is uses a variant of the two lock queue algorithm to provide an efficient queue or list backed by a
 * growable circular array.
 * <p/>
 * Unlike {@link java.util.concurrent.ArrayBlockingQueue}, this class is able to grow and provides a blocking put call.
 * <p/>
 * The queue has both a capacity (the size of the array currently allocated) and a max capacity (the maximum size that
 * may be allocated), which defaults to {@link Integer#MAX_VALUE}.
 * 
 * @param <E>
 *            The element type
 */
public class UnboundArrayBlockingQueue<E> extends AbstractList<E>implements BlockingQueue<E> {

    private final static int cacheLineBytes = 64;
    private final static int integersPerCacheLine = cacheLineBytes >> 2;

    /**
     * The head offset in the {@link #_indexes} array, displaced by 15 slots to avoid false sharing with the array
     * length (stored before the first element of the array itself).
     */
    private static final int HEAD_OFFSET = integersPerCacheLine - 1;
    /**
     * The tail offset in the {@link #_indexes} array, displaced by 16 slots from the head to avoid false sharing with
     * it.
     */
    private static final int TAIL_OFFSET = HEAD_OFFSET + integersPerCacheLine;
    /**
    ...
    private final float _growFactor;
    ...
    private final Lock _tailLock = new ReentrantLock(false);
    private final AtomicInteger _size = new AtomicInteger();
    private final Lock _headLock = new ReentrantLock(true);
     ...
     * @see #DEFAULT_GROWTH
     */
    public UnboundArrayBlockingQueue() {
        _elements = new Object[DEFAULT_CAPACITY];
        _growFactor = DEFAULT_GROWTH;
        _maxCapacity = Integer.MAX_VALUE;
    }

    /**
     * Creates a bounded {@link BlockingArrayQueue} that does not grow. The capacity of the queue is fixed and equal to
     * the given parameter.
     * 
     * @param maxCapacity
     *            the maximum capacity
     */
    public UnboundArrayBlockingQueue(int maxCapacity) {
     ...
     *            the growth factor
     */
    public UnboundArrayBlockingQueue(int capacity, float growFactor) {
     ...
     *            the maximum capacity
     */
    public UnboundArrayBlockingQueue(int capacity, float growFactor, int maxCapacity) {
    ...
    private boolean grow() {
        ...
        try {
            ...
            try {
                ...
                final int newSize = (int) (capacity * _growFactor);
